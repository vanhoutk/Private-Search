...
function tokeniseText(text) {
    // Remove stop words
    var words_string = removeStopWords(text);   // String
    // Stem words - this is slow !
    words_string = stemWords(words_string);       // String
...
function removeStopWords(str) {
    var str2 = str.toLowerCase().replace(/[^a-z]/g, ' ');
  	...
  	for (var i = 0; i < words.length; i++) {
  		var word = words[i];
  		if (word.length > 2 && stopWordsArray.indexOf(word) === -1) {
  			keywords.push(word);
  		}
  	}
