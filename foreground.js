...
/*
function getbirds(doc){
    var birds = doc.getElementsByTagName('li');
    console.log("Birds: " + birds.length);
    for (var i = 0; i < birds.length; i++) {
       // console.log("Bird:" + i + " - "+ birds[i].outerHTML);
        if (birds[i].matches('.ads-ad')) {
...
window.addEventListener('load', function(event) {
    //var timer = setInterval(getbirds(content.document),3000);
    console.log("setting alarm!");
    chrome.alarms.create("CheckPage", {delayInMinutes: 0.1, periodInMinutes: 0.1});
    chrome.alarms.onAlarm.addListener(function( alarm ) {
            console.log("Got an alarm!", alarm);
            console.log ("Ads" + document.getElementsByClassName("ads-ad"));
            });
     browser.alarms.create("CheckPage2", {delayInMinutes: 0.1, periodInMinutes: 0.1});
   browser.alarms.onAlarm.addListener(function( alarm ) {
            console.log("Got an alarm2!", alarm);
            console.log ("Ads2" + document.getElementsByClassName("ads-ad"));
            });


    browser.alarms.onAlarm.addListener(handleAlarm);
*/   //setTimeout (getbirds(content.document),3000);
...
// Generate the observer:
   ...
   // Start observing:
   observer.observe(myElement, {attributes: true,childList: true}); 
...
browser.runtime.onMessage.addListener(request => {
  ...
  if (request.instruction== "CheckAds"){
      //init();
      //chrome.runtime.sendMessage({subject:'test'}, function (rq){ console.log (msg.response)});
...
/*
window.addEventListener('load', function(event) {
  (debug>0)&&log('Add-on loaded...' + document.readyState);
  var page_url = window.location.href;
  log (document.getElementById("gsr").className);
